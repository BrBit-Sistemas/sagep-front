...
export default function FichaCadastralValidacoesPage() {
  ...
  const { data: metrics, isLoading: metricsLoading } = useValidacoesMetrics();

  const { columns } = useValidacoesListTable();
  ...
  return (
    <DashboardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
      <CustomBreadcrumbs
        heading="Validação de Fichas"
        links={[
          { name: 'Laboral' },
          { name: 'Validação de Fichas', href: paths.laboral.fichasCadastraisValidacoes },
        ]}
        sx={{ mb: { xs: 2, md: 3 } }}
      />
